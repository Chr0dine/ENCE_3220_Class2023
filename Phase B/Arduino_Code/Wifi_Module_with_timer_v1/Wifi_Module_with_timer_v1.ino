...
volatile char gCommsBuff[BUFF_SIZE];
volatile char gIncomingChar;
volatile int gCommsBuffInd = 0;
volatile int startCommandFlag = 0;
volatile int endCommandFlag = 0;
...
//timer 2 initialization
  ...
  TCCR3B = 0;
  TCNT3  = 0;
  OCR3A = 624;            // compare match register 16MHz/256
  TCCR3B |= (1 << WGM32);   // CTC mode
  TCCR3B |= (1 << CS32); //1024 prescaler
...
ISR(TIMER3_COMPA_vect){
  if(Serial.available()){
    ...
    if(gIncomingChar == '$'){
      startCommandFlag=1;
      gIncomingChar = Serial.read();
    }else if (gIncomingChar=='\n'){
      startCommandFlag = 0;
      endCommandFlag = 1;
      //or I could put gCommsBuff logic here and get rid of endCommandFlag
    }
     if(startCommandFlag==1){
      gCommsBuff[gCommsBuffInd]=gIncomingChar;
      gCommsBuffInd++;
    }
    }
  }
...
{
    if(setFlag){
      // set Gtime to substring of gCommsBuff
    }else{
...
void loop() {
  ...
  if(endCommandFlag){
    ...
    }else if(strcmp(gCommsBuff,"INC")==0){

    }else if(gCommsBuff.startsWith("SET")){
      setFlag = 1;
    }
  //additional command logic here and clear buffer last
  }
}